...
module.exports = {
  create: async (req, res) => {
    ...
    try {
      ...
      const post = await MovementService.create(body);
  ...
  findOne: async (req, res) => {
    ...
    try {
      const user = await UserService.findOneById(idUser);
      const post = MovementService.findOneByIdInUser(idPost, user);
  ...
  deleteOne: async (req, res) => {
    ...
    try {
      ...
      const post = MovementService.findOneByIdInUser(idPost, user);
      if (!post) res.status(404).json({ message: 'Post not found' });
      await MovementService.updateOneByIdInUser(idPost, user, { is_active: false });
      res.status(204).json();
    } catch (error) {
      res.status(400).json({ message: 'Error getting user post by id', error });
    }
